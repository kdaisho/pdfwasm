import type { CharBox, PageData, SearchMatch, SearchOptions } from "$lib/types";
...
export function findMatches(
	...
	return pages.flatMap((page) => findMatchesInPage(page, query, options));
}

export type SearchCache = WeakMap<
	CharBox[],
	{ key: string; matches: SearchMatch[] }
>;

/**
 * Per-page matches, reusing `cache` for pages whose `chars` array and search
 * parameters are unchanged. Char boxes arrive one page at a time in the
 * background, so without this every arrival would re-scan the whole document.
 * Cached per-page arrays keep their identity, letting consumers skip redraws.
 */
export function findMatchesPerPage(
	pages: PageData[],
	query: string,
	options: SearchOptions,
	cache: SearchCache,
): SearchMatch[][] {
	const params = `${options.caseSensitive}|${options.wholeWord}|${query}`;
	return pages.map((page) => {
		const key = `${page.index}|${params}`;
		const hit = cache.get(page.chars);
		if (hit && hit.key === key) return hit.matches;
		const matches = findMatchesInPage(page, query, options);
		cache.set(page.chars, { key, matches });
		return matches;
	});
}

/** Returns all positions of `query` on a single page. */
