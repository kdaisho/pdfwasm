...
import type { SuggestPage } from "./types.js";
...
/**
 * Pages the model reports as beginning a new unit. Large docs (>=
 * SUGGEST_CHUNK_THRESHOLD) are split into overlapping windows processed in
 * parallel, so the model never has to track page numbers across a long listing
 * (which makes accuracy drift the deeper it gets); smaller docs go in a single
 * call. Boundaries found in any window are merged.
 */
export async function startPages(pages: SuggestPage[]): Promise<Set<number>> {
	const windows =
